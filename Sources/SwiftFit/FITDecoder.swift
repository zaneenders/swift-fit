...
struct FITDecoder: @unchecked Sendable {
  ...
  private func decodeField(
    _ payload: [UInt8], baseType: BaseType,
    bigEndian: Bool
  ) -> [Value] {
    ...
    func loadU16(_ at: Int) -> UInt16 {
      let b0 = UInt16(payload[at])
      let b1 = UInt16(payload[at + 1])
    ...
    for i in 0..<count {
      ...
      case .sint8:
        let v = Int8(bitPattern: loadU8(off))
        result.append(.sint8(v))
      case .uint8:
